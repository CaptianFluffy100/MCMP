...
public class VanillaFrameAllocator extends PortalFrameAllocator {
    ...
    @Override
    public PortalFrameAllocator init(WorldAccess world, BlockPos blockPos, Direction.Axis axis, Block... foundations) {
    ...
    @Override
    public Optional<PortalFrameAllocator> getNewPortal(WorldAccess worldAccess, BlockPos blockPos, Direction.Axis axis, Block... foundations) {
    ...
    @Override
    public Optional<PortalFrameAllocator> getOrEmpty(WorldAccess worldAccess, BlockPos blockPos, Predicate<PortalFrameAllocator> predicate, Direction.Axis axis, Block... foundations) {
        Optional<PortalFrameAllocator> optional = Optional.of(new VanillaFrameAllocator().init(worldAccess, blockPos, axis, foundations)).filter(predicate);
        if (optional.isPresent()) {
            return optional;
        } else {
            Direction.Axis axis2 = axis == Direction.Axis.X ? Direction.Axis.Z : Direction.Axis.X;
            return Optional.of(new VanillaFrameAllocator().init(worldAccess, blockPos, axis2, foundations)).filter(predicate);
        }
    }
    ...
    public void lightPortal(Block frameBlock) {
        PortalDefinition link = MultiversePortalRegistry.getPortalDefinitionFromBlock(frameBlock);
        BlockState blockState = PortalHelper.blockWithAxis(link != null ? link.getPortalBlock().getDefaultState() : MCMPMod.MULTIVERSE_PORTAL_BLOCK.getDefaultState(), axis);
